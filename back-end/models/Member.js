...
module.exports = {
    create:(nom,prenom,user_github,fonction,pdc,mail,admin,point_experience)=>{
        ...
        db.query("INSERT INTO members(nom,prenom,user_github,fonction,pdc,mail,admin,point_experience) values(?,?,?,?,?,?,?,?)",[nom,prenom,user_github,fonction,pdc,mail,admin,point_experience],function(err,resultat){
            if(err){
    ...
    getOneMember: (id) => {
        return new Promise((resolve, reject) => {
            db.query("SELECT * FROM members WHERE id = ?", [id] , function(err, resultat){
              if(err){
                  reject(new Error("Errer resource while fetching membre"));
              }else{
                resolve(resultat)
    ...
    getmemberonproject:()=>{
        return new Promise((resolve, reject) => {
            db.query("SELECT * FROM members WHERE nombre_projet IS NOT NULL", function(err, resultat){
                if(err){
    ...
    getByprenom:(prenom)=>{
        return new Promise((resolve,reject)=>{
            db.query("select members.pdc FROM members WHERE members.prenom=?",[prenom],function(error,res){
                if(error){
                    reject(new Error("Error while fetching member"))
                }else{
                    resolve(res)
    ...
    getPoint:(id_m)=>{
        return new Promise((resolve,reject)=>{
            db.query("SELECT members.point_experience FROM members WHERE members.id=?",[id_m],(err,resultat)=>{
                if(err){
    ...
    setPoint:(point,id)=>{
        return new Promise((resolve,reject)=>{
            db.query("UPDATE members SET point_experience=$1  WHERE members.id=?",[point,id],(err,resultat)=>{
                if(err){
    ...
    setTotproject:(totproj,id)=>{
        return new Promise((resolve,reject)=>{
            db.query("UPDATE members SET nombre_projet =$1 WHERE members.id=?",[totproj,id],(err,resultat)=>{
                if(err){
                    reject(new Error("Error while updating member_point"))
                }else{
                    resolve(resultat)
    ...
    updateMember:(nom,prenom,user_github,fonction,pdc,mail,admin,id)=>{
        return new Promise((resolve,reject)=>{
            db.query("UPDATE members SET nom =?,prenom=?,user_github=?,fonction=?,pdc=?,mail=?,admin=? WHERE id=?",[nom,prenom,user_github,fonction,pdc,mail,admin,id],function(err,resultat){
                if(err){
    ...
    deleteMember:(id)=> {
        return new Promise((resolve, reject) => {
            db.query("DELETE  FROM members WHERE id = ?", [id] , function(err, resultat){
                if(err){
