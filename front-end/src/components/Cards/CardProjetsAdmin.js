...
export default function CardProjetsAdmin(props) {
  ...
  const handleValide =async (nom)=>{
    history.push(`/admin/valide/projet/${nom}`)
            ...
            {projects.map((project)=>( 
              <tr  key={project.id} >
                ...
                <span className="flex items-stretch">
                  ...
                  <button onClick={()=>handleValide(project.nom_projet)} className="bg-emerald-500 text-white active:bg-pink-600 font-bold uppercase text-sm px-2 py-1 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 "
                  >
